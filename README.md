...
3. **Build your file.** Paste the labeled plan into the builder on the site ("2 — Build your file"). The site converts it into its internal configuration, inserts that into the Focus File template, and the file **downloads instantly**. Each download gets a clean session-based name such as `Focus File — Aug 24, 9 AM.html`; the task stays inside the file rather than leaking into its filename. The builder confirms the exact filename without guessing where the browser saved it. An optional, secondary control can create a generic evening reminder in Google Calendar or another calendar. Existing internal JSON plans remain accepted for backward compatibility, but new users never need to see or handle JSON.
...
- **Two numbered actions.** Step 01 shows the five questions and one clear full-width Copy button. Step 02 keeps the textarea visually connected to the matching Create & download button, then replaces it with a compact filename confirmation—no redundant instruction about when to open the file. Keep the optional recurring-reminder invitation secondary, habit-focused, and collapsed. Do not add top-level Answer → Make → Work cards.
