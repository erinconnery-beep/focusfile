...
3. **Build your file.** Paste the labeled plan into the builder on the site ("2 — Build your file"). The site converts it into its internal configuration, inserts that into the Focus File template, and the file **downloads instantly**. The builder then confirms the exact filename and tells the user to open it from Downloads. An optional, secondary control can create a generic evening calendar reminder for making the next Focus File. Existing internal JSON plans remain accepted for backward compatibility, but new users never need to see or handle JSON.
...
- **Two numbered actions.** Step 01 shows the five questions and one clear full-width Copy button. Step 02 keeps the textarea visually connected to the matching Create & download button, then replaces it with a compact filename confirmation. Keep the optional reminder secondary and collapsed. Do not add top-level Answer → Make → Work cards.
...
**Calendar reminders are optional and generic.** The builder creates the `.ics` calendar file locally. It contains only the chosen reminder time, “Make tomorrow’s Focus File,” and a link to focusfile.org—never a task, answer, log, client name, or other Focus File content. After the user adds it, the calendar provider handles the event under its own terms.
