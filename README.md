...
When you're done, a short exit records an explicit status—**Finished**, **Partly finished**, or **Didn’t begin**—plus the progress made, what helped, what pulled you away, and the next step or resume point when one is useful. “Nothing helped” is a valid answer and never becomes a learning signal. The file then turns its saved results into a local **What you're learning** report: repeated helpful cues, repeated distractions, the conditions used, and a concrete carry-forward for the next block. Check-ins stay in the saved results; they are never converted into a score.
...
5. **Record the result.** At the end, choose a completion status and record the progress made, what helped—or that nothing helped—what pulled you off, and the next step. A resume point is required only when the work is unfinished.
...
The learning report defaults to **Today** and shows the actual date behind its block count. The user can switch to the last 7 days, last 14 days, or all history. It compares actual minutes with the adjusted plan and reports deliberate 30-minute extensions separately, so an intentional extra block is not treated as unexplained overtime. **Start report from now…** begins a new report window from that moment without erasing anything; every earlier result remains in the full log. A separate, confirmed **Delete all saved results** action clears the history this file uses in that browser. Downloaded Focus File copies may still contain embedded history and must be deleted separately if the user wants those copies gone. To keep a standalone file responsive, Focus File retains the most recent 100 structured results available to that file.

The Focus File runs entirely in the browser with no internet connection. Its current entries live in browser storage and can be copied or downloaded. If browser storage is disabled, full, or unavailable, the result remains usable in the open tab and the file warns the user to copy or download the log before closing. Private-browsing behavior varies: some browsers keep this storage only until the private session closes, so copy or download anything that must outlast that session.

**Copy next-file interview** defaults to the smallest private scope: the current setup and this block’s result. The user may deliberately expand that to Today, Last 7 days, or All saved results. The learning summary is calculated only from the selected results, and the current setup is always presented as optional guidance the user may keep, change, or ignore. The AI conducts a fresh five-question interview and returns one labeled `FOCUS FILE` plan—never JSON, the HTML template, or a generated HTML file. The user pastes that plan into focusfile.org to download the next file. KEEP is drawn only from concrete help in the most recent included result.
...
That one-copy handoff is intentionally explicit about privacy: the selected information remains local until the user pastes the payload into an AI, at which point that provider’s terms and privacy policy apply. Pressing Copy sends nothing.
...
**After it deploys:** copy the prompt, run it in an AI, paste the returned Focus File plan into the builder, and confirm a Focus File downloads. Complete a sample block and verify that **Copy next-file interview** produces one payload that conducts a fresh five-question interview and returns one labeled plan for focusfile.org—not JSON or HTML. Test all four privacy scopes. Then confirm the Writing, Job search, and Study example links each open a distinct sample file. Clipboard and download work on the live `https://` site.
...
**One exception: AI handoffs.** The AI chat the user chooses sees what is pasted and typed there, under that provider’s own terms and privacy policy. The first setup needs no personal details. The optional next-file handoff defaults to the current setup and one result; the user can deliberately include Today, Last 7 days, or All saved results. The interface states exactly what will be copied before the user pastes it.
