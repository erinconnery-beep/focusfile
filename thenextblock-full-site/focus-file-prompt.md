...
.prestart{text-align:left}
.ps-window{font-family:-apple-system,system-ui,sans-serif;font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:#756f60}
.ps-meta{font-family:-apple-system,system-ui,sans-serif;font-size:12.5px;color:var(--ink-faint);margin-top:7px;font-variant-numeric:tabular-nums}
.ps-meta .cl-sep{color:#c4bdae;margin:0 4px}
.ps-late{color:var(--clay)}
.ps-soon{color:var(--mut)}
...
.btn-finish{background:var(--pine);color:var(--paper);font-family:-apple-system,system-ui,sans-serif;font-weight:700;font-size:17px;padding:17px 40px;border:none;border-radius:40px;cursor:pointer;box-shadow:0 4px 14px rgba(47,106,68,.28)}
...
.alldone{display:none;padding:30px 28px 28px;text-align:center}
.alldone.on{display:block}
.alldone .mark{color:var(--pine);font-size:22px}
.alldone .msg{font-size:24px;color:var(--ink);margin-bottom:22px;display:flex;align-items:center;justify-content:center;gap:10px}
...
.learn-note{font-family:-apple-system,system-ui,sans-serif;font-size:12.5px;color:var(--clay);line-height:1.6;max-width:40ch;margin:26px auto 0}
...
<div class="ps-window" id="psWindow"></div>
...
<button type="button" class="preview-link end-link" id="endLink" style="display:none;">Finish session</button>
</div>
</div>
<!-- BLOCK-END -->
...
<div class="exit-foot">
<span class="exit-log" id="exitLog"></span>
<button class="btn-done" id="doneBtn">Save entry</button>
</div>
</div>
<!-- ALL DONE + LOG -->
<div class="alldone" id="allDone">
<div class="msg"><span class="mark">✓</span> Entry saved.</div>
<button class="btn-buildnext" id="buildNextBtn">Build next block from this →</button>
<div class="buildnext-hint" id="buildNextHint">One click copies today's session log together with the setup prompt&mdash;paste it into a new chat and the next block starts where this one ended.</div>
...
<div class="learn-note">The log is for learning, not judging. Look for the patterns and habits worth changing, and carry one into the next block. Over time, ritual becomes habit.</div>
...
const c = $("counter");
c.textContent = done + " / " + total + " COMPLETE";
...
previewing = false;
stopAsking();
$("checkin").style.marginTop = "";
$("pull").classList.remove("on");
if (!previewWasVisible) $("checkin").classList.remove("on");
}
...
on = !on;
document.title = on ? "\u25CF Still on task?" : BASE_TITLE;
}, 900);
}
...
function startAsking(isPreview) {
$("checkinLabel").textContent = "Are you on task?";
$("checkin").classList.add("asking");   // dot pulses clay, reason shows
knockBeep();
knockFlash();
if (!isPreview) startTitleBlink();       // real knock only: blink the tab for when they're in another window
}
function stopAsking() {
...
let preTimer = null;
function plannedStartStr() { return parsePlanned(cfg.scope || "").start; }
function plannedStartDate() {
const raw = plannedStartStr();
const m = raw && raw.match(/(\d{1,2}):(\d{2})\s*(am|pm)?/i);
if (!m) return null;
...
const raw = plannedStartStr();
const st = plannedStartDate();
$("psWindow").textContent = cfg.scope || "";
let meta = "now " + nowHHMM();
if (raw && st) {
const diff = Math.round((Date.now() - st.getTime()) / 60000);
let late, cls;
if (diff < 0) { late = "in " + fmtDur(-diff); cls = "ps-soon"; }
else if (diff === 0) { late = "it's time \u2014 begin"; cls = "ps-late"; }
else { late = fmtDur(diff) + " past your " + raw + " start"; cls = "ps-late"; }
meta += ' <span class="cl-sep">\u00b7</span> <span class="' + cls + '">' + late + '</span>';
}
$("psMeta").innerHTML = meta;
}
function renderClockLine() {
const el = $("clockLine");
if (!el || !startTime) return;
const parts = [];
parts.push("Started " + fmtClock(new Date(startTime)));
...
ctx.push("- In Q2, if \"" + (e.nextStart || e.task) + "\" reads as a real task, propose continuing from it (they can change it). If it is vague, gibberish, or unclear, ignore it and ask what they are working on as usual.");
...
var _ps = plannedStartStr();
$("beginBtn").innerHTML = (_ps ? "Begin \u2014 you said " + _ps : "Begin session") + ' <span class="arr">\u2192</span>';
renderPreStart();
preTimer = setInterval(renderPreStart, 1000);
...
previewing = false;                         // clear any pre-start preview so real knocks count
$("checkin").style.marginTop = "";
...
if (previewing) { endPreview(); return; }   // any answer ends the preview, back to the original screen
if (!startTime) return;                // no session yet: buttons are inert
...
if (previewing) return;                     // already showing the demo
previewing = true;
previewWasVisible = $("checkin").classList.contains("on");
$("checkin").classList.add("on");           // show it if pre-start
if (!previewWasVisible) $("checkin").style.marginTop = "22px";  // gap from the Begin button (pre-start only)
startAsking(true);                          // flash, beep, asking — no tab blink. Stays until an answer is tapped.
});
}
function selfTest() {
