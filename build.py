...
Flow: copy the short prompt -> AI interview -> AI outputs a Focus File plan (JSON config)
-> paste it into the builder on index.html -> it fetches focus-file.html, injects the config,
and downloads the Focus File HTML.
...
def main():
    cname=(ROOT/'CNAME')
    if not cname.exists() or cname.read_text().strip()!='www.focusfile.org':
        sys.exit('build: CNAME must contain www.focusfile.org')
    ...
    print('Deploy: index.html + CNAME + focus-file-logo.svg + focus-file.html + sample-writing.html + sample-jobsearch.html + sample-study.html')
